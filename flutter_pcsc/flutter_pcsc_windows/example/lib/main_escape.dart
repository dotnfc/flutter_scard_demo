...
class Message {
  ...
  static highlight(String content) {
...
class _MyAppBodyState extends State<MyAppBody> {
  ...
  Future<void> getCardSerialNumber() async {
    int ctx = await PcscPlatform.instance
        .establishContext(PcscConstants.CARD_SCOPE_SYSTEM /*CARD_SCOPE_USER*/);
    Map? card;
    try {
      ...
      for (String reader in readers) {
        setState(() {
          _messages.add(Message.info('Found reader: $reader'));
        });
      }
      ...
      if (readers.isEmpty) {
        ...
        setState(() {
          _messages.add(Message.highlight('Using reader: $reader'));
        });

        card = await PcscPlatform.instance.cardConnect(ctx, reader,
            //PcscConstants.SCARD_SHARE_SHARED, PcscConstants.SCARD_PROTOCOL_ANY);
            PcscConstants.SCARD_SHARE_DIRECT, PcscConstants.SCARD_PROTOCOL_UNDEFINED);

        // var response = await PcscPlatform.instance.transmit(card['h_card'], card['active_protocol'], getCardSerialNumberCommand);
        var response = await PcscPlatform.instance.escapeCommand(card['h_card'], getCardSerialNumberCommand);
