...
class Pcsc {
  ...
  /// Reset card.
  static Future<List<int>> reset(CardStruct card, bool cold) async {
    return _platform.reset(card.hCard, cold);
  }
  ...
  /// Send an APDU to the ccid reader.
  static Future<List<int>> escape(CardStruct card, List<int> commandBytes,
      {bool newIsolate = false}) {
    return _platform.escapeCommand(card.hCard, commandBytes, newIsolate: newIsolate);
  }
